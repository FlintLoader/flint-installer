...
public final class FlintService {
	...
	private static <A, R> R invokeWithFallbacks(Handler<A, R> handler, A arg) throws IOException {
		...
		do {
			FlintService service = Reference.FLINT_SERVICES[index];
			...
			index = (index + 1) % Reference.FLINT_SERVICES.length;
	...
	private interface Handler<A, R> {
		R apply(FlintService service, A arg) throws IOException;
	}
	...
	public static void setFixed(String metaUrl, String mavenUrl) {
		...
		activeIndex = -1;
		fixedService = new FlintService(metaUrl, mavenUrl);
	}

	FlintService(String meta, String maven) {
		this.meta = meta;
		this.maven = maven;
	}
